...
{
	auto& sprite_info = sprites_data.at(i_sprite_name);
	auto& sprite = sprite_info.sprite;
	sf::Rect<short> texture_box(sprite_info.texture_box);
	...
	sprite->setOrigin(sprite_info.origin.x, sprite_info.origin.y);
	sprite->setPosition(static_cast<sf::Vector2f>(i_position));
